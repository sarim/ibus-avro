...
AvroRegex.prototype = {
    parse: function(input) {
        ...
        // Initialize Map once
        if (!this.data.patternMap) {
            this.data.patternMap = {};
            this.data.maxPatternLen = 0;
            for (var i = 0; i < this.data.patterns.length; i++) {
                var pattern = this.data.patterns[i];
                this.data.patternMap[pattern.find] = pattern;
                if (pattern.find.length > this.data.maxPatternLen) {
                    this.data.maxPatternLen = pattern.find.length;
                }
            }
        }
        
        var maxLen = this.data.maxPatternLen;
        var map = this.data.patternMap;
        ...
        for(var cur = 0; cur < len; ++cur) {
            ...
            for(var chunkLen = maxLen; chunkLen >= 1; --chunkLen) {
                var end = start + chunkLen;
                if (end <= len) {
                    var chunk = fixed.substring(start, end);
                    var pattern = map[chunk];
                    if (pattern) {
                        var prev = start - 1;
